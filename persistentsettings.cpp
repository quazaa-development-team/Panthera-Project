...
void PersistentSettings::loadSettings()
{
	...
	Settings.WebServices.BitziWebView = m_qSettings.value("BitziWebView", "http://bitzi.com/lookup/(URN)?v=detail&ref=panthera").toString();
	...
	Settings.Ares.EnableAlways = m_qSettings.value("EnableAlways", true).toBool();
	Settings.Ares.EnableToday = m_qSettings.value("EnableToday", true).toBool();
...
void PersistentSettings::saveSettings()
{
	...
	m_qSettings.setValue("EnableToday", Settings.Ares.EnableToday);
